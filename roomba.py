...
class Position(object):
    ...
    def getTimeToPos(self, other, speed):
        ...
        diag_dist = ((x_dist**2) + (y_dist**2))**0.5
        return diag_dist/speed
    ...
    def getBearing(self, other):
        ...
        incL = new_y- old_y
        X = math.cos(new_x) * math.sin(incL)
        Y = (math.cos(old_x) * math.sin(new_x)) - (math.sin(old_x) * math.cos(new_x) * math.cos(incL))
        angle = math.degrees(math.atan2(X, Y))
        if angle < 0:
            angle += 360
        return round(angle,0)

    def getNewPosition(self, angle, speed, time = 1):
        """
        ...
        delta_y = speed * math.sin(math.radians(angle))* time
        delta_x = speed * math.cos(math.radians(angle))*time
...
class RectangularRoom(object):
    ...
    def __init__(self, width, height):
        ...
        height: an integer > 0
        """
        self.room_map = {tile : 0 for tile in product(range(height), range(width))}
        self.width = width
        self.height = height
    ...
    def cleanTileAtPosition(self, pos):
        ...
        if self.room_map[tile] == 0:
            self.room_map[tile] = 1
    ...
    def isTileCleaned(self, m, n):
        ...
        """
        tile = (m, n)
        return True if self.room_map[tile] else False
    ...
    def getNumCleanedTiles(self):
        ...
        return len([value for value in self.room_map.values() if value])
...
# === Your own Robot
class GraphDrivenRobot(Robot):
    '''Uses graph search algorithm to find the shortest path
...
#
#       (... your call here ...)
    showPlot1("robots", "num_robots", "time")
